...
const save_data = (file, object) => {
    ...
    const stats = fs.statSync('persistence/' + file)
    ...
    const old_array = fs.readFileSync('persistence/' + file).toString()
...
const write_file = (file, data) => {
    fs.writeFileSync('persistence/' + file, data)
}

const read_file = (file) => {

    const stats = fs.statSync('persistence/' + file)
    const array = fs.readFileSync('persistence/' + file).toString()
